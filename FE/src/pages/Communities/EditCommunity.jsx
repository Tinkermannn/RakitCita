...
export default function EditCommunityPage() {
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 max-w-2xl">
            <button onClick={() => navigate(`/communities/${communityId}`)} className="mb-6 inline-flex items-center text-orange-600 hover:text-orange-800">
                <ArrowLeft size={20} className="mr-2" /> Kembali ke Detail Komunitas
            </button>
            <div className="bg-white p-8 rounded-lg shadow-xl">
                <h1 className="text-3xl font-bold text-gray-800 mb-2 flex items-center">
                    <Save size={30} className="mr-3 text-orange-500" /> Edit Komunitas
                </h1>
                <p className="text-gray-600 mb-8">Perbarui detail komunitas Anda.</p>

                <form onSubmit={handleSubmit} className="space-y-6">
                    {error && <div className="text-sm text-red-600 bg-red-100 p-3 rounded-md">{error}</div>}
                    
                    <div>
                        <label htmlFor="name_community_edit" className="block text-sm font-medium text-gray-700 mb-1">Nama Komunitas <span className="text-red-500">*</span></label>
                        <input type="text" name="name" id="name_community_edit" value={formData.name} onChange={handleChange} className="input-field" required/>
                    </div>

                    <div>
                        <label htmlFor="description_community_edit" className="block text-sm font-medium text-gray-700 mb-1">Deskripsi (Opsional)</label>
                        <textarea name="description" id="description_community_edit" rows="4" value={formData.description} onChange={handleChange} className="input-field"></textarea>
                    </div>
                    
                    <div>
                        <label htmlFor="bannerImage_edit" className="block text-sm font-medium text-gray-700 mb-1">Gambar Banner (Ganti jika perlu)</label>
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                            <div className="space-y-1 text-center">
                                ...
                                <div className="flex text-sm text-gray-600">
                                    <label htmlFor="bannerImage_edit" className="relative cursor-pointer bg-white rounded-md font-medium text-orange-600 hover:text-orange-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-orange-500">
                                ...
                                {formData.bannerImage && <p className="text-xs text-green-600 mt-1">{formData.bannerImage.name} (File baru dipilih)</p>}
                                {!formData.bannerImage && currentBannerUrl && <p className="text-xs text-blue-600 mt-1">Menggunakan banner saat ini.</p>}
                            </div>
                        </div>
                    </div>
