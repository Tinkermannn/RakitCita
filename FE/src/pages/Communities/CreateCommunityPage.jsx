        ...
        <div className="container mx-auto p-4 md:p-8 max-w-2xl">
            ...
            <div className="bg-white p-8 rounded-lg shadow-xl">
                <h1 className="text-3xl font-bold text-gray-800 mb-2 flex items-center">
                    <Users size={30} className="mr-3 text-orange-500" /> Buat Komunitas Baru
                </h1>
                <p className="text-gray-600 mb-8">Bangun ruang untuk berdiskusi dan berbagi dengan minat yang sama.</p>

                <form onSubmit={handleSubmit} className="space-y-6">
                    {error && <div className="text-sm text-red-600 bg-red-100 p-3 rounded-md">{error}</div>}
                    
                    <div>
                        <label htmlFor="name_community" className="block text-sm font-medium text-gray-700 mb-1">Nama Komunitas <span className="text-red-500">*</span></label>
                        <input type="text" name="name" id="name_community" value={formData.name} onChange={handleChange} className="input-field" required placeholder="Contoh: Pecinta Fotografi Difabel"/>
                    </div>

                    <div>
                        <label htmlFor="description_community" className="block text-sm font-medium text-gray-700 mb-1">Deskripsi (Opsional)</label>
                        <textarea name="description" id="description_community" rows="4" value={formData.description} onChange={handleChange} className="input-field" placeholder="Jelaskan tentang komunitas ini, tujuannya, dan siapa saja yang bisa bergabung."></textarea>
                    </div>
                    
                    <div>
                        <label htmlFor="bannerImage" className="block text-sm font-medium text-gray-700 mb-1">Gambar Banner (Opsional)</label>
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                            <div className="space-y-1 text-center">
                                ...
                                <div className="flex text-sm text-gray-600">
                                    <label htmlFor="bannerImage" className="relative cursor-pointer bg-white rounded-md font-medium text-orange-600 hover:text-orange-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-orange-500">
                                ...
                                {formData.bannerImage && <p className="text-xs text-green-600 mt-1">{formData.bannerImage.name}</p>}
                            </div>
                        </div>
                    </div>
